...
//		val 只读变量（常量）  var 可修改变量
        ...
        val hfg = mutableMapOf<Int, String>()
...
//		return a>b?a:b //没有3目运算符的,只能用if else
    ...
    fun test14() {
        ...
        val str2 = str1?.let {  //用it  返回最后
            val sd4 = 3
            "4t"
        }
        val str3 = str1.run {   //用this  返回最后
            toString()
            4
        }
        val str6 = with(str1) { //用this  返回最后
            5
        }
        val str4 = str1.apply {  //初始化 返回自身
            4
        }
        val str5 = str1.also {    //更新  返回自身
            5
        }
    }
    ...
    //also 等价于apply it  返回对象本身 用于执行对象操作 可以链式调用
...
//		val str3 = str2 //不能使用作用域的变量
    ...
    private fun test22() {
        ...
        //1map函数 副本 遍历每一一个元素，进行改造 输出一个新list，原来的list不变
