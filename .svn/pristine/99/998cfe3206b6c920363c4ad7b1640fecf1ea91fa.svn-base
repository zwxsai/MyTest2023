 ...
 * Describer:数据库更新辅助类---工具类不用修改
 * 原理：创建临时表-->删除原表-->创建新表-->复制临时表数据到新表并删除临时表；这样数据库表的更新就完成了
 */


public final class MigrationHelper2 {
    ...
    public static void migrate(SQLiteDatabase db, ReCreateAllTableListener listener, Class<? extends AbstractDao<?,
            ?>>... daoClasses) {
        weakListener = new WeakReference<>(listener);
        migrate(db, daoClasses);
    }

    public static void migrate(Database database, ReCreateAllTableListener listener, Class<? extends AbstractDao<?,
            ?>>... daoClasses) {
        weakListener = new WeakReference<>(listener);
        migrate(database, daoClasses);
    }
     ...
     */
    private static void reflectMethod(Database db, String methodName, boolean isExists, @NonNull Class<?
